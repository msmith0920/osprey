"""Tests for the bundled demo logbook seed data.

Validates that the standalone demo logbook JSON is well-formed and compatible
with GenericJSONAdapter for tutorial use. The demo uses *relative* timestamps
(``when: {days_ago, time}``) resolved at ingest time, so seeded entries always
land at a recent, deterministic position without mutating the source file.

(The ``control_assistant`` preset no longer ships a standalone demo logbook —
its logbook narrative lives in self-contained simulation scenario bundles,
seeded via ``osprey sim apply``; see tests/simulation/test_scenario_bundles.py.)
"""

import json
from datetime import UTC, datetime
from pathlib import Path

DEMO_DATA_PATH = (
    Path(__file__).resolve().parent.parent.parent.parent
    / "src/osprey/templates/apps/ariel_standalone/data/logbook_seed/demo_logbook.json"
)


class TestDemoLogbookData:
    """Tests for the bundled standalone demo logbook seed data."""

    def test_demo_file_exists(self):
        """Demo logbook JSON file exists in template."""
        assert DEMO_DATA_PATH.exists(), "Demo logbook file missing from template"

    def test_demo_file_is_valid_json(self):
        """Demo logbook file is valid JSON with sufficient entries."""
        data = json.loads(DEMO_DATA_PATH.read_text())
        assert "entries" in data
        assert len(data["entries"]) >= 15, "Need enough entries for meaningful search"

    def test_demo_entries_have_required_fields(self):
        """Each demo entry has fields required by GenericJSONAdapter (relative when)."""
        data = json.loads(DEMO_DATA_PATH.read_text())
        for entry in data["entries"]:
            assert "id" in entry, f"Entry missing 'id': {entry}"
            when = entry.get("when")
            assert isinstance(when, dict), f"Entry {entry.get('id')} missing relative 'when'"
            assert isinstance(when.get("days_ago"), int) and when["days_ago"] >= 0
            assert "time" in when, f"Entry {entry.get('id')} 'when' missing 'time'"
            assert "text" in entry or "title" in entry, (
                f"Entry missing text content: {entry.get('id')}"
            )

    def test_demo_entries_parseable_by_adapter(self):
        """Demo entries parse via GenericJSONAdapter and resolve to recent timestamps."""
        from osprey.services.ariel_search.config import ARIELConfig
        from osprey.services.ariel_search.ingestion.adapters.generic import (
            GenericJSONAdapter,
        )

        data = json.loads(DEMO_DATA_PATH.read_text())
        config = ARIELConfig.from_dict(
            {
                "database": {"uri": "postgresql://mock/test"},
                "ingestion": {
                    "adapter": "generic_json",
                    "source_url": str(DEMO_DATA_PATH),
                },
            }
        )
        adapter = GenericJSONAdapter(config)
        now = datetime.now(UTC)

        for entry_data in data["entries"]:
            result = adapter._convert_entry(entry_data)
            assert result["entry_id"], f"Missing entry_id for {entry_data['id']}"
            assert result["raw_text"], f"Missing raw_text for {entry_data['id']}"
            assert result["timestamp"], f"Missing timestamp for {entry_data['id']}"
            # Relative timestamps resolve to the recent past (within ~1 month).
            age_days = (now - result["timestamp"]).days
            assert 0 <= age_days <= 31, f"{entry_data['id']} resolved to {result['timestamp']}"
