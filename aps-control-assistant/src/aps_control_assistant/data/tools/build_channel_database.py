...
def build_database(csv_path: Path, output_path: Path, use_llm: bool = False, config_path: Path = None):
    ...
    if standalone:
        if use_llm:
            print(f"\n🤖 Generating descriptive names for {len(standalone)} standalone channels using LLM...")
            try:
        ...
        # Generate names
            ...
            print(f"\n📝 Using addresses as channel names for standalone channels")
            generated_names = [ch.get('address', '') for ch in standalone]
    ...
    print(f"  📊 Total entries: {len(db['channels'])}")
