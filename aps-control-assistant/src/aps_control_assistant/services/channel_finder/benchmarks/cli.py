#!/usr/bin/env python
"""
Benchmark CLI for aps-control-assistant
...
Usage:
    python src/aps_control_assistant/services/channel_finder/benchmarks/cli.py
    python src/aps_control_assistant/services/channel_finder/benchmarks/cli.py --queries 0:10
"""
...
from aps_control_assistant.services.channel_finder.benchmarks.runner import BenchmarkRunner
from aps_control_assistant.services.channel_finder.utils.config import get_config
...
async def run_benchmarks(
    ...
    if not verbose:
        logging.getLogger('aps_control_assistant').setLevel(logging.WARNING)
    ...
    console.print(
        Panel(
            ...
            f"[dim]aps-control-assistant[/dim]",
            style=Styles.HEADER,
    ...
    try:
        ...
        results_table.add_row("📁 Results location", f"src/aps_control_assistant/data/benchmarks/results/")
        ...
        console.print()
        console.print(f"  [{Styles.INFO}]ℹ️  Check the output files for detailed metrics and results[/{Styles.INFO}]")
        console.print()
...
Examples:
  ...
  python -m aps_control_assistant.services.channel_finder.benchmarks.cli --queries 0,5,10,15

  # Use custom benchmark dataset file
  python -m aps_control_assistant.services.channel_finder.benchmarks.cli --dataset src/aps_control_assistant/data/benchmarks/datasets/my_custom.json
