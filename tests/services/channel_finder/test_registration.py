...
class MockCustomDatabase(BaseDatabase):
    ...
    def load_database(self):
        """Load mock data."""
        self.channels = [
            {"channel": "MockChannel1", "address": "MOCK:CH1", "description": "Mock channel 1"},
            {"channel": "MockChannel2", "address": "MOCK:CH2", "description": "Mock channel 2"},
        ]
    ...
    def format_chunk_for_prompt(self, chunk: list[dict], include_addresses: bool = False) -> str:
        ...
        for ch in chunk:
            if include_addresses:
                formatted.append(
                    f"- {ch['channel']} (Address: {ch['address']}): {ch['description']}"
                )
            else:
...
def temp_config_file():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        json.dump({"test_config": "value", "custom_option": "from_file"}, f)
        config_path = f.name
        ...
        "channel_finder": {
            ...
            "pipelines": {
                "mock_custom": {
                    ...
                    "processing": {"custom_param": "test_param"},
                }
            },
        },
        "models": {
        ...
        "channel_finder": {
            ...
            "pipelines": {
                "in_context": {
                    ...
                    "processing": {"chunk_dictionary": False, "max_correction_iterations": 2},
                }
            },
        },
        "models": {
...
def test_unknown_database_error(temp_config_file, monkeypatch):
    ...
    mock_config = {
        "channel_finder": {
            ...
            "pipelines": {
                "in_context": {
                    "database": {"type": "nonexistent_db", "path": temp_config_file},
                    "processing": {},
                }
            },
        },
