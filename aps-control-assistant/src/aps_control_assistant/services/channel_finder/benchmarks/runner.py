...
from aps_control_assistant.services.channel_finder.service import ChannelFinderService
...
class BenchmarkRunner:
    ...
    def __init__(self):
        ...
        console.print(f"  [{Styles.INFO}]🔌 Loading channel finder service...[/{Styles.INFO}]", end="")
        try:
    ...
    async def run_query_multiple_times(
        ...
        for run_num in range(1, num_runs + 1):
            ...
            if result.success:
                console.print(f"    [{Styles.SUCCESS}]✓[/{Styles.SUCCESS}] Found {len(result.found_pvs)} PV(s) in {result.execution_time_seconds:.2f}s")
            else:
            ...
            try:
                ...
                async with self.results_lock:
                    ...
                    if queries_completed > 0 and query_times:
                        ...
                        console.print(f"  [{Styles.INFO}]⏱️  {queries_completed}/{len(selected_entries)} completed ({progress_pct:.1f}%)[/{Styles.INFO}] | "
    ...
    async def _save_incremental_results_async(
        ...
        results_dir_str = output_config.get('results_dir', 'src/aps_control_assistant/data/benchmarks/results')
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            for eval in evaluations:
                status = "✓" if eval.best_f1_score == 1.0 else "⚠" if eval.best_f1_score > 0 else "✗"
        ...
        if not is_complete:
            console.print(f"  [{Styles.DIM}]💾 Saved incremental results to: {json_path.name}[/{Styles.DIM}]")
