...
from aps_control_assistant.services.channel_finder.llm.completion import get_chat_completion
...
- If description mentions "set value", "set point", "settable", "writable" → use "SetPoint"
- If description mentions "read back", "read only", "readback", "measured" → use "ReadBack"
...
"""
    ...
    def resolve_duplicates(self, channels: List[Dict], names: List[str]) -> List[str]:
        ...
        print(f"\n⚠️  Found {len(duplicates)} duplicate name(s) affecting {total_dups} channels:")
        for dup_name, indices in duplicates.items():
            print(f"   '{dup_name}': {len(indices)} channels")

        print(f"🔄 Regenerating names with more specificity...")
        ...
        try:
            ...
            print(f"   ✓ Resolved all duplicates")
            return result
