...
def test_extract_multiple_same_type_contexts(context_manager, mock_state):
    ...
    step = {
        'inputs': [
            {"CURRENT_WEATHER": "sf_weather"},
            {"CURRENT_WEATHER": "ny_weather"}
        ]
    }
...
def test_extract_with_hard_constraints_multiple_same_type(context_manager, mock_state):
    ...
    # Store contexts
    context_manager.set_context("PV_ADDRESSES", "pvs1", PVAddressesContext(pvs=["PV1"]), skip_validation=True)
    context_manager.set_context("PV_ADDRESSES", "pvs2", PVAddressesContext(pvs=["PV2"]), skip_validation=True)
    context_manager.set_context("TIME_RANGE", "time", TimeRangeContext(start="2025-01-01"), skip_validation=True)
...
def test_extract_with_soft_constraints_multiple_same_type(context_manager, mock_state):
    ...
    context_manager.set_context("ARCHIVER_DATA", "data1", ArchiverDataContext(data=[1, 2, 3]), skip_validation=True)
    context_manager.set_context("ARCHIVER_DATA", "data2", ArchiverDataContext(data=[4, 5, 6]), skip_validation=True)
...
def test_extract_from_step_with_mixed_context_types(context_manager, mock_state):
    ...
    context_manager.set_context("PV_ADDRESSES", "pvs", PVAddressesContext(pvs=["PV1"]), skip_validation=True)
    context_manager.set_context("TIME_RANGE", "time", TimeRangeContext(start="2025-01-01"), skip_validation=True)
    context_manager.set_context("ANALYSIS_PARAMS", "params", AnalysisParamsContext(method="fft"), skip_validation=True)
...
def test_extract_from_step_mixed_single_and_multiple(context_manager, mock_state):
    ...
    context_manager.set_context("TIME_RANGE", "time", TimeRangeContext(start="2025-01-01"), skip_validation=True)
    context_manager.set_context("PV_ADDRESSES", "set1", PVAddressesContext(pvs=["PV1"]), skip_validation=True)
    context_manager.set_context("PV_ADDRESSES", "set2", PVAddressesContext(pvs=["PV2"]), skip_validation=True)
...
def test_extract_from_step_no_constraints_returns_dict(context_manager, mock_state):
    ...
    context_manager.set_context("ARCHIVER_DATA", "data1", ArchiverDataContext(data=[1, 2, 3]), skip_validation=True)
    context_manager.set_context("PV_VALUES", "vals", PVValuesContext(values={}), skip_validation=True)
...
def test_extract_from_step_emptiness_check_still_works(context_manager, mock_state):
    ...
    # Non-empty case
    context_manager.set_context("ARCHIVER_DATA", "test", ArchiverDataContext(data=[]), skip_validation=True)
...
def test_pattern_3_capabilities_iterate_dict_values(context_manager, mock_state):
    ...
    context_manager.set_context("ARCHIVER_DATA", "data1", ArchiverDataContext(data=[1, 2]), skip_validation=True)
    context_manager.set_context("ARCHIVER_DATA", "data2", ArchiverDataContext(data=[3, 4]), skip_validation=True)
...
def test_cardinality_multiple_constraint_passes(context_manager, mock_state):
    ...
    contexts = context_manager.extract_from_step(
        step, mock_state,
        constraints=[("PV_ADDRESSES", "multiple")],
        constraint_mode="hard"
    )
...
def test_cardinality_invalid_value_raises_error(context_manager, mock_state):
    ...
    with pytest.raises(ValueError) as exc_info:
        context_manager.extract_from_step(
            step, mock_state,
            constraints=[("PV_ADDRESSES", "invalid")],
            constraint_mode="hard"
        )
...
def test_cardinality_archiver_use_case(context_manager, mock_state):
    ...
    context_manager.set_context("PV_ADDRESSES", "pvs", PVAddressesContext(pvs=["PV1", "PV2"]), skip_validation=True)
    context_manager.set_context("TIME_RANGE", "range", TimeRangeContext(start="2025-01-01", end="2025-01-02"), skip_validation=True)
...
def test_cardinality_with_soft_constraint_mode(context_manager, mock_state):
    ...
    context_manager.set_context("ARCHIVER_DATA", "data", ArchiverDataContext(data=[1, 2, 3]), skip_validation=True)
