...
- Runtime panel control for the web terminal — the agent can show/hide configured panels and register ad-hoc URL panels at runtime (`show_panel`, `hide_panel`, `register_panel`; `list_panels` now reports per-panel visibility and the active tab). Panels can be declared launched-but-hidden with `web.panels.<id>.hidden: true`. Runtime URL registration is off by default behind `web.allow_runtime_panels`, with an optional SSRF-validated `web.runtime_panel_allowlist`.
...
- Removed the non-functional `on_violation` knob from `control_system.limits_checking` (and the shipped `config.yml` templates). It was never honoured — every channel-limits violation has always blocked the write — but the templates advertised a `"skip"` mode implying violations would be let through. Limit enforcement is unconditional and fail-closed; an `on_violation` key left in an existing config is now simply ignored.
...
- The ARIEL web upload route (entries with attachments) now publishes its text body through the facility adapter with the same credential and publish-failure handling as text-only entries — previously it always saved local-only and never attempted to publish. Attachments are stored in ARIEL; because the OLOG write API cannot accept file uploads, a published entry's response states explicitly that the files were saved to ARIEL only. Adapters declare whether publishing needs credentials via a new fail-closed `requires_write_auth` property (the local JSON adapter opts out; ALS requires it), and the web create form adapts its credential prompt to the configured logbook via a new `GET /api/publish-info` endpoint — showing the credential fields only when the logbook needs them, and otherwise saying whether the entry will publish without credentials or be saved to ARIEL only.
- **`claude-agent-sdk` upgraded to 0.2.106** (bundles CLI 2.1.185); `uv.lock` regenerated to match the pin (#278).

### Fixed

- ARIEL `semantic_search` MCP tool now degrades gracefully instead of returning a hard error when semantic search is unavailable (no pgvector embedding table / Ollama). The service returns an empty result that steers the agent to `keyword_search`, matching the documented "degrades gracefully to keyword-only" contract; previously agents calling the tool hit a tool failure (#276).
- Workspace gallery now reliably shows new artifacts without a manual refresh. The store index was written in place (`open(path, "w")`), so the gallery's cross-process file watcher — which reloads without taking the lock — could read a half-written file, log "Could not load artifacts index; starting fresh", and blank the gallery until the next refresh. The index is now written atomically (temp file + `os.replace`), and the watcher detects the resulting rename (`on_moved`) so the new entry appears immediately (#289).
- Channel Finder in-context Explore filter now searches the whole database and re-chunks the filtered results, instead of filtering only the currently displayed page. Previously a search match on a later page read as "No channels match the filter" while the pager still showed multiple chunks, so operators had to page through chunks by hand to find it (#299).
- ARIEL web Browse pagination now works: `/api/entries` derives an `offset` from `page` and passes it to the repository, so the Browse view's Previous/Next buttons page through older entries instead of always returning the first page. The endpoint's declared `author` and `source_system` query filters are now honored too (both were previously accepted and silently ignored), and the reported `total_pages` is computed from the same filtered count rather than the whole table.
- `osprey ariel migrate` now creates an embedding table for every model in `enhancement_modules.text_embedding.models`, not just the hardcoded default (`nomic-embed-text`, 768). The migration runner instantiated `TextEmbeddingMigration` with no arguments, so configured models (e.g. `mxbai-embed-large`/1024) were dropped and a later `osprey ariel enhance` found no matching table and produced no embeddings. Previously only `osprey ariel reembed` honoured non-default models (via its `--model`/`--dimension` flags).
- The `control-assistant` **EVENTS** tab now renders in `osprey web`. The event-dispatcher dashboard is a URL-backed panel (`web.panels.events.url`), but `events` was registered as a *built-in* panel, so the web terminal discarded its URL and the frontend had no built-in events tab — the tab never appeared and `EVENT_DISPATCHER_URL` was dead. `events` is now a custom URL-backed panel, so the tab renders and health-gates against the dispatcher's `/health` as documented.
- The EPICS connector now routes Channel Access through the configured `gateways.write_access` gateway when `control_system.writes_enabled` is true, falling back to `gateways.read_only` otherwise. Previously it always used `read_only` for both reads and writes, so a deployment with a genuine read-only gateway had its hardware writes rejected. EPICS uses one process-wide CA context, so the connector points at a single gateway; keeping writes-disabled deployments on the read-only gateway reinforces the `writes_enabled` master switch at the network layer.
- The channel-limits `defaults` block is now actually inherited. A channel that omits a field (`writable`, `min_value`/`max_value`, `max_step`, or `verification`) now picks it up from the database's top-level `defaults` block; previously `defaults` was validated but silently dropped, so a `defaults: {writable: false}` lockdown had no effect and such channels fell back to `writable: true`. A channel's own fields still override the defaults.
- Dispatched agent runs now fail fast with a clear error when the model provider never responds — an invalid/expired provider credential or an unreachable base URL. A per-message inactivity watchdog (`DISPATCH_INACTIVITY_SEC`, default 120s) aborts a silently hung run instead of stalling to the worker's 300s dispatch timeout and surfacing only a generic "timed out". Each streamed SDK message resets the clock, so healthy runs are unaffected.
- Creating an ARIEL logbook entry no longer silently saves a local-only copy when publishing to the facility logbook needs credentials or genuinely fails. A logbook that requires credentials now returns HTTP 401 and the web form prompts for the username/password (keeping the entry populated for resubmission) instead of reporting "Entry created"; a real publish failure (wrong password, logbook unreachable) surfaces an error. Only a genuinely read-only adapter still falls back to a local-only save.
- Web-uploaded (ARIEL-only) attachments on a published entry are no longer erased by a later background re-ingestion poll. Because the OLOG write API cannot accept file uploads, the upstream copy of a published entry has no attachments, and the periodic re-ingest used to overwrite the entry's attachment references to empty — orphaning the stored files. An upsert that carries no attachments now preserves the existing ones (a non-empty incoming list still replaces them, so upstream attachments stay authoritative).
- `provider=ds4` (local DeepSeek V4) now resolves in the `control_assistant` and `hello_world` presets — the `api.providers` stanza was missing from the app templates, so the Claude Code resolver rejected it.
