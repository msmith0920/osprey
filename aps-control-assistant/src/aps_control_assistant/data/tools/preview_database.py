...
from aps_control_assistant.services.channel_finder.utils.config import get_config, resolve_path
from aps_control_assistant.services.channel_finder.databases import TemplateChannelDatabase, HierarchicalChannelDatabase
...
def preview_hierarchical(db_path: str, show_full: bool = False):
    ...
    if not show_full:
        console.print()
        console.print(
            f"[info]💡 Tip:[/info] Use [bold]--full[/bold] flag to see complete hierarchy"
        )
    ...
    console.print(
        Panel.fit(
            f"[success]✓ Preview complete! [bold]{total_channels}[/bold] total channels across "
...
def preview_in_context(db_path: str, presentation_mode: str, show_full: bool = False):
    ...
    if not show_full and len(all_channels) > 20:
        ...
        console.print(
            ...
            f"[info]💡 Tip:[/info] Use [bold]--full[/bold] flag to see all channels"
        )
